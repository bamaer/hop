...
public class GraphOutputDialog extends BaseTransformDialog implements ITransformDialog {
  ...
  private MetaSelectionLine<GraphDatabaseMeta> wConnection;
  ...
  private void validateAndWarn(GraphOutputMeta input) {
    ...
    //
    try {
      IHopMetadataSerializer<GraphDatabaseMeta> connectionSerializer =
          metadataProvider.getSerializer(GraphDatabaseMeta.class);
      GraphDatabaseMeta connection = connectionSerializer.load(input.getConnection());
      if (connection == null) {
        message.append(Const.CR);
        message.append(
            BaseMessages.getString(
